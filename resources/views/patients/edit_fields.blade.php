...
<div class="row mt-3">
    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            {{ Form::text('address1', isset($patient->address->address1) ? $patient->address->address1 : null, ['class' => 'form-control']) }}
        </div>
    </div>
    <div class="col-md-6">
        <div class="form-group">
            {{ Form::label('address2', __('messages.user.address2').':') }}
            {{ Form::text('address2', isset($patient->address->address2) ? $patient->address->address2 : null, ['class' => 'form-control']) }}
        </div>
    </div>
    <div class="col-md-3">
        <div class="form-group">
            {{ Form::label('city', __('messages.user.city').':') }}
            {{ Form::text('city', isset($patient->address->city) ? $patient->address->city : null, ['class' => 'form-control']) }}
        </div>
    </div>
    <div class="col-md-3">
        <div class="form-group">
            {{ Form::label('zip', __('messages.user.zip').':') }}
            {{ Form::text('zip', isset($patient->address->zip) ? $patient->address->zip : null, ['class' => 'form-control','onkeyup' => 'if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,"")', 'maxlength' => '6','minlength' => '6']) }}
        </div>
    </div>
