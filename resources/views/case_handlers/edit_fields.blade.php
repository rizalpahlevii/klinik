...
<div class="row mt-3">
    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            {{ Form::text('address1', isset($caseHandler->address->address1) ? $caseHandler->address->address1 : null, ['class' => 'form-control']) }}
        </div>
    </div>
    <div class="col-md-6">
        <div class="form-group">
            {{ Form::label('address2', __('messages.user.address2').':') }}
            {{ Form::text('address2', isset($caseHandler->address->address2) ? $caseHandler->address->address2 : null, ['class' => 'form-control']) }}
        </div>
    </div>
    <div class="col-md-3">
        <div class="form-group">
            {{ Form::label('city', __('messages.user.city').':') }}
            {{ Form::text('city', isset($caseHandler->address->city) ? $caseHandler->address->city : null, ['class' => 'form-control']) }}
        </div>
    </div>
    <div class="col-md-3">
        <div class="form-group">
            {{ Form::label('zip', __('messages.user.zip').':') }}
            {{ Form::text('zip', isset($caseHandler->address->zip) ? $caseHandler->address->zip : null, ['class' => 'form-control','onkeyup' => 'if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,"")', 'maxlength' => '6','minlength' => '6']) }}
        </div>
    </div>
