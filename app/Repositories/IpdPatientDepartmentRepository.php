...
class IpdPatientDepartmentRepository extends BaseRepository
{
    ...
    {
        $data['patients'] = Patient::with('user')->get()->where('user.status', '=', 1)->pluck('user.full_name', 'id');;
        $data['doctors'] = Doctor::with('user')->get()->where('user.status', '=', 1)->pluck('user.full_name', 'id');
        $data['bedTypes'] = BedType::pluck('title', 'id');
     ...
     * @param  bool  $isEdit
     * @param  integer  $bedId
     * @param  integer  $ipdPatientBedTypeId
     *
     * @return Collection
     */
    public function getPatientBeds($bedTypeId, $isEdit, $bedId, $ipdPatientBedTypeId)
    {
        $beds = null;
        if (! $isEdit) {
            $beds = Bed::where('bed_type', $bedTypeId)->where('is_available', 1)->pluck('name', 'id');
        } else {
            $beds = Bed::where('bed_type', $bedTypeId)->where('is_available', 1);
